<?php
/*********************************************************************************
 * InitPHP 3.6 国产PHP开发框架  扩展类库-文件上传
 *-------------------------------------------------------------------------------
...
class uploadInit {
	...
	private function parseParams(array $params) {
		...
		$temp['allowFileType'] = (isset($params['allowFileType'])) ? $params['allowFileType'] : $this->defaultAllowFileType;
		return $temp;
	}
	...
	private function getImgSize($srcFile, $srcExt = null) {
		...
		!isset($srcdata['width']) && list($srcdata['width'], $srcdata['height'], $srcdata['type']) = @getimagesize($srcFile);
